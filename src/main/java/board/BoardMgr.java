...
public class BoardMgr {
	...
	public void insertBoard(HttpServletRequest req) {
		...
		try {
			...
			sql = "insert tblBoard(name, content, subject, ref, pos, "
	...
	public int getTotalCount(String keyField, String keyWord) {
		...
		try {
			...
			if(keyWord.trim().equals("")||keyWord==null) {
				...
				//검색 일때, keyFiled : name, subject, content
				sql = "select count(*) from tblBoard "
						+ "where " + keyField + " like ?";//like '%aaa%'
				pstmt = con.prepareStatement(sql);
	...
	//Board Read : 하나의 게시물, 13개 컬럼 빈즈에 모두 저장
	...
	//Board Update : 업로드 파일까지 수정 기능 추가
	...
	public void replyBoard(BoardBean bean) {
		...
		try {
			...
			//depth : 답변의 깊이 원글:0,답변:1,답변의답변:2....
